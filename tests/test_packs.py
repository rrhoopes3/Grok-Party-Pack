...
class TestBuiltinPacks:
    def test_all_packs_load(self):
        """All 7 built-in packs should be loadable."""
        packs = [RESEARCH_PACK, BUILDER_PACK, OPS_PACK, TRADING_PACK, ARENA_PACK, EMAIL_PACK, SALESFORCE_PACK]
        names = {p.name for p in packs}
        assert names == {"research", "builder", "ops", "trading", "arena", "email", "salesforce"}
...
class TestGlobalRegistry:
    def test_get_registry_returns_populated(self):
        """Global registry should have all 7 built-in packs."""
        # Reset singleton for clean test
        import forge.packs as packs_mod
        packs_mod._registry = None
        reg = get_registry()
        assert len(reg.list_packs()) == 7
        assert set(reg.list_names()) == {"research", "builder", "ops", "trading", "arena", "email", "salesforce"}
